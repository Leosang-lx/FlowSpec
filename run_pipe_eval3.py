    ...
    for question_path in run_config.question_paths:
        ...
        for _ in range(run_config.error_repeat):
            ...
            if rank == 0:
                ...
                print(f'temperature: {args.temperature}, pipeline_type: {args.pipeline_type}, question_path: {question_path}, question_begin: {run_config.question_begin}, question_end: {run_config.question_end}')
                print(f'throughput: {throughput}, avg_latency: {avg_latency}')
                if run_config.eval_record:
                    with open(record_path, 'a') as f:
                        f.write(f'temperature: {args.temperature}, pipeline_type: {args.pipeline_type}, question_path: {question_path}, question_begin: {run_config.question_begin}, question_end: {run_config.question_end}\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--temperature", type=float, default=run_config.temperatures[0])
    parser.add_argument("--pipeline_type", type=str, default=run_config.pipeline_types[0])
